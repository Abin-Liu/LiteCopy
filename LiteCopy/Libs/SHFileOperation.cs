...
{	
	/// <summary>
	/// Win32API SHFileOperation，实现文件/文件夹的复制，移动，删除和重命名
	/// </summary>
	public sealed class SHFileOperation
	{
        ...
        {
            return CopyOrMove(sourceFiles, targetFiles, false);
        }
		#endregion
        ...
        {
            return CopyOrMove(sourceFiles, targetFiles, true);
        }
		#endregion
        ...
        /// <param name="recycleBin">是否进入回收站，如果为false文件将被彻底删除</param>
        /// <returns>0表示成功，其余为错误码，可通过GetErrorString()方法获取错误内容</returns>
        public static int Delete(string filePath, bool recycleBin = true)
        {
            ...
            return _SHFileOperation(lpFileOp);
        }
		#endregion
		...
		/// <returns></returns>
		[DllImport("shell32.dll", SetLastError = true, CharSet = CharSet.Unicode, EntryPoint = "SHFileOperation")]
		private static extern int _SHFileOperation(SHFILEOPSTRUCT lpFileOp);
		...
		/// <returns>0表示成功，其余为错误码，可通过GetErrorString()方法获取错误内容</returns>
		private static int CopyOrMove(IEnumerable<string> sourceFiles, IEnumerable<string> targetFiles, bool deleteSourceFile)
		{
			if (sourceFiles.Count() != targetFiles.Count())
			{
				throw new ArgumentException("File count unmatch: sourceFiles and targetFiles");
			}

			SHFILEOPSTRUCT pm = new SHFILEOPSTRUCT();

			pm.pFrom = string.Join(FILE_SPLITER, sourceFiles) + FILE_SPLITER;
			pm.pTo = string.Join(FILE_SPLITER, targetFiles) + FILE_SPLITER;
			...
			if (true == deleteSourceFile)
			{
				...
				pm.lpszProgressTitle = "Moving files";
			}
			else
			{
				pm.wFunc = wFunc.FO_COPY;
				pm.lpszProgressTitle = "Copying files";
			}

			return _SHFileOperation(pm);
		}
        ...
        private static void InitErrorMap()
        {
			...
			ErrorMap.Add("0", "ERROR_SUCCESS : The operation completed successfully.");
